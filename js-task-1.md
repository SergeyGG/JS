...
12. Некая сеть фастфудов предлагает несколько видов гамбургеров:
    ...
    **Комментарии**. Эта задача вызывает много непонимания, потому внимательно прочти эти комментарии перед решением. 
    ...
    - имена методов начинаются с глагола и имеют вид «сделайЧтоТо»: `countCalories()`, `addTopping()`
    ...
    **Послесловие**. Если ты внимательно читал учебник по JS, то наверно знаешь, что в JS нет классов, а до версии ES5 нет и констант (~~а как же решать задачу?~~). Классы в JS имитируются разными споcобами, самый общеупотребимый - через добавление методов в прототипы объекта: 
...
- WebRTC — стандарт, который позволяет браузерам соединяться друг с другом напрямую (peer-to-peer) и обмениваться данными, файлами, звуком и видео. Да, вполне можно сделать свой аналог скайпа, не требующий установки.
